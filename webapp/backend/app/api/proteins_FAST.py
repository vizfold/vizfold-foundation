...
def fetch_pdb_background(protein_id: int, protein_name: str, protein_dir: str, db_url: str):
    ...
    if pdb_fetch.is_valid_pdb_id(protein_name):
        try:
            ...
            try:
                ...
                if protein:
                    ...
                    db.commit()
                print(f"✓ Successfully fetched PDB file for {protein_name}")
            finally:
                db.close()
        except Exception as e:
            print(f"✗ Could not fetch PDB file for {protein_name}: {e}")
...
async def create_protein(
    ...
    FAST VERSION: Returns immediately, fetches PDB in background if applicable
    """
    ...
    # Create protein in database (FAST)
    db_protein = Protein(
        name=protein.name,
        sequence=sequence,
    ...
    # CRITICAL FIX: Fetch PDB in background, don't block the response
    if PDBFetchService().is_valid_pdb_id(protein.name):
