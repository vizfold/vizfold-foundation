...
Updated Usage:
    python generate_tsne_from_pickle.py <pickle_file> [--output_dir] OUTPUT_DIR [--layer LAYER] [--interactive] [--output_dir] OUTPUT_DIR

Example:
    python generate_tsne_from_pickle.py demo_outputs/demo_protein_intermediate_reps.pt --layer 47
    
Updated Example: 
    python generate_tsne_from_pickle.py demo_outputs/demo_protein_intermediate_reps.pt --layer 47 --interactive --output_dir outputs/tsne_from_pickle_interactive
"""
...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend for matplotlib paths
...
def _normalize_rep_keys(reps: dict) -> dict:
    """
    Handle legacy/alternate keys from saved pickle files.
    Expected downstream keys: 'msa', 'pair', 'single'.
    """
    if any(k in reps for k in ['msa', 'pair', 'single']):
        return reps

    # Fallback for naming like msa_layers/pair_layers/final_msa/final_pair
    normalized = {}
    ...
    # If final layer tensors exist but layer dicts are empty, synthesize a single entry
...
def generate_tsne_visualization(
    pickle_file,
    output_dir: str = 'outputs/tsne_from_pickle',
    layer_idx: int = None,
    methods=None,
    ...
    Args:
        ...
        interactive: Use Plotly and save HTML outputs when True
    """
    if methods is None:
    ...
    if layer_idx not in available_layers:
        ...
        # Fallback to last available layer to keep workflow running
    ...
    print(f"  {data.shape[0]} residues x {data.shape[1]} features")
    ...
    for method, reduced in results.items():
        ...
        save_path = os.path.join(output_dir, f'{method}_layer{layer_idx}.{ext}')
        if interactive:
            plot_2d_embedding_interactive(
                reduced,
    ...
    if len(results) > 1:
        if interactive:
            comparison_dir = os.path.join(output_dir, 'comparison_html')
            ...
            print(f"  Saved interactive comparison plots to: {comparison_dir}")
        else:
    ...
    print("\nFiles created:")
    print(f"  - Individual plots: {', '.join([f'{m}_layer{layer_idx}' for m in results.keys()])}.{ 'html' if interactive else 'png'}")
    if len(results) > 1:
        label = 'comparison_html' if interactive else f'comparison_layer{layer_idx}.png'
        print(f"  - Comparison plot: {label}")
...
Examples:
  ...
  python generate_tsne_from_pickle.py my_protein_reps.pt --methods pca tsne
    ...
    parser.add_argument('pickle_file', type=str, help='Path to pickle file (.pt) with intermediate representations')
    parser.add_argument('--output_dir', type=str, default='outputs/tsne_from_pickle', help='Directory to save output visualizations')
    parser.add_argument('--layer', type=int, default=None, help='Layer index to visualize (default: last layer)')
    parser.add_argument('--methods', nargs='+', choices=['pca', 'tsne', 'umap'], default=['pca', 'tsne', 'umap'], help='Dimensionality reduction methods to apply')
    parser.add_argument('--interactive', action='store_true', help='Use Plotly and save interactive HTML visualizations instead of PNGs')
